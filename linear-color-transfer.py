...
target_img = imread(args.target_image)[:, :, :3].astype(float) / 255.0
source_img = imread(args.source_image)[:, :, :3].astype(float) / 255.0
output_img = match_color(target_img, source_img, mode=args.mode, eps=float(args.eps))
