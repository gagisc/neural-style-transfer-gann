...
def match_color(target_img, source_img, mode='pca', eps=1e-5):
    ...
    if mode == 'chol':
        ...
        raise ValueError('Unknown mode "%s". Valid options are: chol, pca, sym.' % mode)
...
if cp_mode == 'lum':
    style_img = imread(args.style_image)[:, :, :3].astype(float) / 255.0
    content_img = imread(args.content_image)[:, :, :3].astype(float) / 255.0
    ...
    org_content = imread(args.org_content)[:, :, :3].astype(float) / 255.0
    output = imread(args.output_lum2)[:, :, :3].astype(float) / 255.0
    org_content = resize(org_content, output.shape)
    ...
    raise ValueError('Unknown colour preservation mode "%s". Valid options are: lum, lum2, match, match_style.' % cp_mode)
